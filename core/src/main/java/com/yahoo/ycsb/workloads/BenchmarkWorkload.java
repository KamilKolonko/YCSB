 ...
 * </ul>
 */
public class BenchmarkWorkload extends Workload {

  //protected ListOrderedMap tables;
  protected List<Entity> entities;
  protected Map<String, String[]> tableHeaders = new HashMap<>();
  private List<Entity> generatedEntities = new ArrayList<>();

  //private List<Integer> tablesToInsert;
  ...
  public void init(Properties p) throws WorkloadException {
    entities = Utils.readDatabases(".\\core\\src\\main\\resources\\com\\yahoo\\ycsb\\database\\tables", tableHeaders);

    fieldlengthgenerator = BenchmarkWorkload.getFieldLengthGenerator(p);
  ...
  /**
   * Builds a value for a single field specified in {@param field} parameter.
   */
  private HashMap<String, ByteIterator> buildSingleValue(String key, String field) {
  ...
  /**
   * Builds values for all fields specified in {@param fields} parameter.
   */
  private HashMap<String, ByteIterator> buildValues(String key, String[] fields) {
   ...
   * <p>
   * This function is executed during YCSB loading phase.
   */
  @Override
  public boolean doInsert(DB db, Object threadstate) {
    ...
    if (keynum >= entities.size()) {
      System.err.println("All database values are already inserted. Loading phase completed.");
      return false;
    }
    ...
    int numOfRetries = 0;
    do {
      status = db.insert(entities.get(keynum).getTableName(), dbkey,
          entities.get(keynum).getData(tableHeaders.get(entities.get(keynum).getTableName())));
  ...
  protected void verifyRow(HashMap<String, ByteIterator> expected, HashMap<String, ByteIterator> actual) {
    Status verifyStatus = Status.OK;
    long startTime = System.nanoTime();
    if (actual != null && !actual.isEmpty()) {
      for (Map.Entry<String, ByteIterator> entry : expected.entrySet()) {
        if (!entry.getValue().toString().equals(actual.get(entry.getKey()).toString())) {
          verifyStatus = Status.UNEXPECTED_STATE;
          break;
        }
      }
  ...
  protected int nextKeynum() {
    ...
    if (keychooser instanceof ExponentialGenerator) {
      ...
      do {
        //default is random number between insertstart and insertcount
  ...
  public void doTransactionRead(DB db) {
    ...
    Entity entity;
    if (keynum >= entities.size()) {
      entity = generatedEntities.get(keynum % entities.size());
    } else {
      entity = entities.get(keynum);
    }
    ...
    if (!readallfields) {
      ...
      fields.add(getRandomFieldName(tableHeaders.get(entity.getTableName())));
    } else if (dataintegrity) {
      // pass the full field list if dataintegrity is on for verification
      fields = new HashSet<String>(Arrays.asList(tableHeaders.get(entity.getTableName())));
    }

    HashMap<String, ByteIterator> cells = new HashMap<String, ByteIterator>();
    db.read(entity.getTableName(), keyname, fields, cells);

    if (dataintegrity) {
      verifyRow(entity.getData(tableHeaders.get(entity.getTableName())), cells);
    }
  }
  ...
  public void doTransactionReadModifyWrite(DB db) {
    ...
    if (writeallfields) {
      ...
      values = buildValues(keyname, tableHeaders.get(entity.getTableName()));
    } else {
      // update a random field
      values = buildSingleValue(keyname, getRandomFieldName(tableHeaders.get(entity.getTableName())));
    }
    ...
    db.update(entity.getTableName(), keyname, values);
  ...
  public void doTransactionScan(DB db) {
    ...
    String tableName;
    if (keynum >= entities.size()) {
      tableName = generatedEntities.get(keynum % entities.size()).getTableName();
    } else {
      tableName = entities.get(keynum).getTableName();
    }
    ...
    if (!readallfields) {
      // read a random field
      fields = new HashSet<String>();
      fields.add(getRandomFieldName(tableHeaders.get(tableName)));
    }

    db.scan(tableName, startkeyname, len, fields, new Vector<HashMap<String, ByteIterator>>());
  }
  ...
  public void doTransactionUpdate(DB db) {
    ...
    if (writeallfields) {
      // new data for all the fields
      values = buildValues(keyname, tableHeaders.get(tableName));
    } else {
      // update a random field
      values = buildSingleValue(keyname, getRandomFieldName(tableHeaders.get(tableName)));
    }
  ...
  public void doTransactionInsert(DB db) {
    ...
    try {
      ...
      if (keynum >= entities.size()) {
        ...
        HashMap<String, ByteIterator> values = buildValues(dbkey, tableHeaders.get(tableName));
        db.insert(tableName, dbkey, values);
        generatedEntities.add(new Entity(tableName, values.values()));
      } else {
        db.insert(entities.get(keynum).getTableName(), dbkey,
            entities.get(keynum).getData(tableHeaders.get(entities.get(keynum).getTableName())));
      }
    } finally {
  ...
  private String getRandomTableName() {
    int randomTableIndex = ThreadLocalRandom.current().nextInt(0, tableHeaders.keySet().size());
    return (String) tableHeaders.keySet().toArray()[randomTableIndex];
  }

  private String getRandomFieldName(String[] columns) {
    int randomColumnIndex = ThreadLocalRandom.current().nextInt(0, columns.length);
    return columns[randomColumnIndex];
  }
